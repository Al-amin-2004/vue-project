...
<template>
  <aside
    class="p-4 max-w-20 h-full bg-gray-900 text-slate-300 border-e border-gray-400 flex flex-col justify-between"
  >
    <div class="flex flex-col items-center gap-4 pb-4 border-b border-slate-200/40">
    ...
    <div v-for="(item) in SidebarDatas" :key="item.id" v-on:click="MyFunc(item.id)" class="flex flex-col items-center cursor-pointer gap-1" :class="{'text-green-400': item.id === active }">
